...
void main(void) {
    ...
    CF_UART_setPrescaler(UART1, 42);
    CF_UART_setTxFIFOThreshold(UART1, 3);
    CF_UART_enableTx(UART1);
    CF_UART_enableRx(UART1);
    send_pulse(1);

    CF_UART_writeCharArr(UART0, "UART0: Test\n");
    send_pulse(1);

    CF_UART_writeCharArr(UART1, "UART1: Test\n");
    send_pulse(1);

    uint32_t invalid_read = USER_readWord(0x20000 >> 2);
    if (invalid_read == 0xDEADBEEF) {
        send_pulse(2);
    } else {
        send_pulse(1);
    }
